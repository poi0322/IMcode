...
//hw5- p2
//grammar_data 도 수정
char keyword[19][50] = { "if", "else", "while", "do", "for", "include", "define", "typedef", "struct", "int", "char", "float", "double", "void", "return", "case", "then", "true", "false" };
int total_keywords = 19;	//keyword 수 수정
int idx_First_keyword = 31;	//the token index for the first keyword in the keyword table
...
typedef struct nodetype
{
	...
	int rule_no_used;
	char place[20], begin[12], next[12], tru[12], fal[12];
	char* code;
} nodeT;
...
int main()
{
	...
	char source_file_name[30];
	printf("Please type the name of the source file: ");
	scanf("%s", source_file_name);
	fps = fopen(source_file_name, "r");
	if (!fps)
	{
		printf("File open of source file is failed\n");
		getchar();
	}
...
void code_gen(Ty_Node_Ptr cur)
{
	Ty_Node_Ptr L_, L, L1, S, S1, S2, id_t, id1_t, id2_t, rop, BE, BE1, BT, BT1, BF, BF1, E, E1, T, T1, F, num_t;

	if (cur->nodesym.kind == 0) return;
	...
	case 5:		// S -> if ( BE ) then S1 else S2
		...
		strcpy(BE->tru, newlabel());
		strcpy(BE->fal, newlabel());
		strcpy(S1->begin, BE->tru);
		strcpy(S1->next, S->next);
		strcpy(S2->begin, BE->fal);
		strcpy(S2->next, S->next);
		code_gen(BE);
		code_gen(S1);
		code_gen(S2);
	...
	case 16:	//BE -> BT
		...
		BE = cur;
		BT = cur->children[0];
		strcpy(BT->tru, BE->tru);
		strcpy(BT->fal, BE->fal);
		code_gen(BT);
	...
	case 17:	//BE -> BE1 || BT
		...
		BE1 = cur->children[0];
		BT = cur->children[2];
		strcpy(BE1->tru, BE->tru);
		strcpy(BE1->fal, newlabel());
		strcpy(BT->tru, BE->tru);
		strcpy(BT->fal, BE->fal);
		code_gen(BE1);
		code_gen(BT);
	...
	case 18:	//BT -> BF
		BT = cur;
		BF = cur->children[0];
		strcpy(BF->tru, BT->tru);
		strcpy(BF->fal, BT->fal);
		/////////////////p16 빈칸
		code_gen(BF);
	...
	case 19:	//BT -> BT1 && BF
		...
		BT1 = cur->children[0];
		BF = cur->children[2];
		strcpy(BT1->fal, BT->fal);
		strcpy(BT1->tru, newlabel());
		strcpy(BF->fal, BT->fal);
		strcpy(BF->tru, BT->tru);
		code_gen(BT1);
		code_gen(BF);
		BT->code = strcat_my(BT1->code, BT->tru, ":\n", BF->code, STR0, STR0, STR0);
		/////////////////////p16 빈칸???????????????
		break;

	case 20:	//BF -> true
		BF = (Ty_Node_Ptr)malloc(sizeof(nodeT)); 
	...
	case 22:	//BF -> id1 rop id2
		BF = cur;
		id1_t = cur->children[0];
		rop = cur->children[1];
		id2_t = cur->children[2];
		BF->code = strcat_my2("if ", id1_t->nodesym.str, " ", rop->nodesym.str, " ", id2_t->nodesym.str, " goto ", BF->tru, "\ngoto ", BF->fal);
		strcat(BF->code, "\n");
	...
	case 23:	// BF -> ( BE )
		BF = cur;
		BE = cur->children[1];
		strcpy(BE->tru, BF->tru);
		strcpy(BE->fal, BF->fal);
		code_gen(BE);
	...
	case 24:	/// BF -> ! BF1
		BF = cur;
		BF1 = cur->children[1];
		strcpy(BF1->tru, BF->fal);
		strcpy(BF1->fal, BF->tru);
		code_gen(BF1);
